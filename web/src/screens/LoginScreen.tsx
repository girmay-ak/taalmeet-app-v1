...
export function LoginScreen({ onLogin, onSignup }: LoginScreenProps) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Success - navigation will be handled by the hook or parent component
  ...
  const handleGoogleLogin = async () => {
    try {
      // OAuth login will be implemented when OAuth is configured
      toast.info('Google login coming soon!');
      // For now, just trigger the login callback
      onLogin();
    } catch (err: any) {
  ...
  const isLoading = signInMutation.isPending || forgotPasswordMutation.isPending;
  ...
  return (
    <div className="relative min-h-screen w-full bg-[#0A0A0A] overflow-hidden">
      ...
      {/* Content */}
      <div className="relative z-10 min-h-screen w-full flex flex-col justify-center items-center px-6 py-12">
        <div className="w-full max-w-md">
          ...
          {/* Error Message */}
          {error && (
            <div className="mb-4 p-4 bg-red-500/10 border border-red-500/20 rounded-xl flex items-start gap-3 animate-slide-down">
          ...
          {showForgotPassword ? (
            <div className="space-y-4 mb-6 animate-slide-up">
              ...
              <button
                ...
                className="w-full py-3 text-[#9CA3AF] hover:text-white transition-colors"
              >
                Back to Login
              </button>
            </div>
            ...
            <>
              ...
              <div className="space-y-3 mb-8 animate-slide-up" style={{ animationDelay: '0.3s' }}>
                <button
                  ...
                  disabled={isLoading}
                  className="w-full py-4 bg-white text-[#0F0F0F] rounded-xl font-medium flex items-center justify-center gap-3 active:scale-[0.98] transition-transform disabled:opacity-50"
                >
                ...
                <button
                  ...
                  onClick={onLogin}
                  disabled={isLoading}
                  className="w-full py-4 bg-[#1A1A1A] border border-[#2A2A2A] text-white rounded-xl font-medium flex items-center justify-center gap-3 active:scale-[0.98] transition-transform disabled:opacity-50"
                >
          ...
          <div className="text-center animate-fade-in" style={{ animationDelay: '0.4s' }}>
            <p className="text-[#9CA3AF]">
              ...
              <button
                ...
                onClick={onSignup}
                disabled={isLoading}
                className="text-[#1DB954] font-semibold hover:text-[#1ED760] transition-colors disabled:opacity-50"
              >
                Sign up
              </button>
            </p>
